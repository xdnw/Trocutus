...
public class Activity {
    ...
    public static Function<DBKingdom, Activity> createCache(int turns) {
        HashMap<DBKingdom, Activity> activityMap = new HashMap<>();
        return nation -> activityMap.computeIfAbsent(nation, (Function<DBKingdom, Activity>) n -> n.getActivity(turns));
    }
    ...
    private void load(Set<Long> activity) {
        ...
        for (long turn = minTurn; turn <= currentTurn; turn++) {
            ...
            ZonedDateTime other = now.minusHours(turnDiff * 1);

            DayOfWeek day = other.getDayOfWeek();
            int dayTurn = other.getHour() / 1;
            int weekTurn = dayTurn + day.ordinal() * 24;
    ...
    public Activity(int nationId, long inactiveTurns) {
        Set<Long> activity = Trocutus.imp().getDB().getActivity(nationId, TimeUtil.getTurn() - inactiveTurns);
        if (activity.isEmpty()) return;
    ...
    public Activity(int nationId) {
        Set<Long> activity = Trocutus.imp().getDB().getActivity(nationId, 0);
        if (activity.isEmpty()) return;
